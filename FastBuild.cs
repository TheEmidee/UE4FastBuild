...
namespace UnrealBuildTool
{
    class FASTBuild : ActionExecutor
    {
        ...
        private string CachePath = @"\\STORAGE2\cache\FASTBuildCache";
        ...
        private void WriteEnvironmentSetup()
        {
            ...
            try
            {
                ...
                {
                    string windows_sdk_version = "Latest";
                    string compiler_version = "Latest";

                    // Force 2017
                    var compiler = WindowsCompiler.VisualStudio2017;// WindowsPlatform.GetDefaultCompiler(null);

                    VCEnv = VCEnvironment.Create(compiler, CppPlatform.Win64, compiler_version, windows_sdk_version);
                }
            ...
            var install_dir = VCEnv.ToolChainDir;
            ...
            if (VCEnv != null)
            {
                ...
                string sdk_base_path = Path.Combine(VCEnv.WindowsSdkDir.ToString(), "bin", VCEnv.WindowsSdkVersion.ToString(), "x64" );

                AddText(string.Format(".WindowsSDKBasePath = '{0}'\n", sdk_base_path));

                AddText("Compiler('UE4ResourceCompiler') \n{\n");
                AddText("\t.Executable = '$WindowsSDKBasePath$/rc.exe'\n");
                ...
                // Reconstruct the path to the Redist folder
                string redist_version_number = "14.15.26706";
                string redist_path = Path.Combine(install_dir.ToString(), "../../../", "Redist/MSVC/", redist_version_number );

                AddText(string.Format("\t\t'{0}/x64/Microsoft.VC141.CRT/msvcp{1}.dll'\n", redist_path, platformVersionNumber));
        ...
        private void AddCompileAction(Action Action, int ActionIndex, List<int> DependencyIndices)
        {
            ...
            else if (ParsedCompilerOptions.ContainsKey("/Yu")) //Use PCH
            {
                ...
                CompilerOutputExtension = extension + ".obj";
            }
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        string extension = Path.GetExtension(InputFile);
                        CompilerOutputExtension = extension + ".o";
                    }
                }
            }
